...
!function (a) {
    ...
    (function (a) {
        return function () {
            ...
            // No any white spaces at the beginning and end
            a.validator.addMethod("noWhitespaceAtBeginEnd", function (value, element) {
                var res = /^\S.*\S$/.test(value);
                return res;
            }, "Whitespace characters are not allowed at begin and end of this field."),

            // No any spacebars inside this field, except begin and end
            a.validator.addMethod("noSpaces", function (value, element) {
                return value.indexOf(" ") === -1; // Check if the value contains no spaces
            }, "Spaces are not allowed in this field."),

            // No whitespaces anywhere in this field
            a.validator.addMethod("nowhitespace", function (a, b) {
                return this.optional(b) || /^\S+$/i.test(a)
            }, "Whitespace characters are not allowed at this field."),
            a.validator.addMethod("noWhiteSpace", function (value, element) {
                return value.indexOf(" ") < 0 && value != "" || value == "";
            }, "Whitespace characters are not allowed at this field."),

            // Not are whitespaces
            a.validator.addMethod("isNotOnlyWhiteSpaces", function (value, element) {
                var valid = true;
                ...
                return valid;
            }, "Please dont enter only whitespaces."),

            // Allowed english characters + numbers, underscores but disallowed special characters, spaces
            a.validator.addMethod("alphanumeric", function (a, b) {
                return this.optional(b) || /^\w+$/i.test(a)
            }, "Letters, numbers, and underscores only please."),

            a.validator.addMethod("startWithLetter", function (a, b) {
                return this.optional(b) || /^^[A-Za-z]/.test(a)
            }, "Must start with a letter please."),

            // Allowed english characters + numbers, spaces, underscores but disallowed special characters          
            a.validator.addMethod("englishSpacesUnderscoresChars", function (value, element) {
                return /^[A-Za-z0-9\s_]*$/.test(value);
            }, "Please enter only letters, numbers, underscores, and spaces."),

            // Allowed english characters + numbers, underscores but disallowed special characters          
            a.validator.addMethod("englishUnderscoresChars", function (value, element) {
                return /^[A-Za-z0-9\s_]*$/.test(value);
            }, "Please enter only letters, numbers, underscores."),

            // Allowed vietnamese characters, spaces but disallowed special characters
            a.validator.addMethod("vietnameseSpacesChars", function (value, element) {
                return /^[qwertyuiopQWERTYUIOPèéẻẽẹÈÉẺẼẸềếểễệỀẾỂỄỆỳýỷỹỵỲÝỶỸỴùúủũụÙÚỦŨỤìíỉĩịÌÍỈĨỊòóỏõọÒÓỎÕỌồốổỗộỒỐỔỖỘaAàáảãạÀÁẢÃẠằắẳẵặẰẮẮẴẶầấẩẫậẦẤẨẪẬsSdDđĐfFgGhHjJkKlLzZxXcCvVbBnNmM\s]+$/i.test(value);
            }, "Please enter only Vietnamese characters and spaces."),

            // Allowed vietnamese characters + numbers, spaces but disallowed special characters
            a.validator.addMethod("vietnameseNumbersSpacesChars", function (value, element) {
                return /^[0123456789qwertyuiopQWERTYUIOPèéẻẽẹÈÉẺẼẸềếểễệỀẾỂỄỆỳýỷỹỵỲÝỶỸỴùúủũụÙÚỦŨỤìíỉĩịÌÍỈĨỊòóỏõọÒÓỎÕỌồốổỗộỒỐỔỖỘaAàáảãạÀÁẢÃẠằắẳẵặẰẮẮẴẶầấẩẫậẦẤẨẪẬsSdDđĐfFgGhHjJkKlLzZxXcCvVbBnNmM\s]+$/i.test(value);
            }, "Please enter only Vietnamese characters, numbers and spaces."),

            // Must contains at least one number        
            a.validator.addMethod("atLeastOneNumber", function (value, element) {
                return /\d/.test(value);
            }, "Must contains at least one number please."),

            // Must contains at least one special character i.e: ~`!@#$%^&*()_-+={{[}}]|\\:;\"'<,>?/.
            a.validator.addMethod("atLeastOneSpecialChar", function (value, element) {
                return /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(value);
            }, "Must contains at least one special character i.e: ~`!@#$%^&*()_-+={{[}}]|\\:;\"'<,>?/. please."),

            // Must contains at least one uppercase letter and at least one lowercase letter        
            a.validator.addMethod("atLeastOneUpperLetterAndLowerLetter", function (value, element) {
                return /^(?=.*[a-z])(?=.*[A-Z]).+$/.test(value);
            }, "Must contains at least one uppercase letter and at least one lowercase letter please."),
